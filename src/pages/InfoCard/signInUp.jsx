...
export default function SingInUp(props) {
    ...
    const [cardPacket, setCardPacket] = useState(2);
        ...
        API.post("infocard/getInfocard",{id : id})
            .then((result) => {
                ...
                if(result.data.status == 'success'){
                    ...
                    setHeaderDesc(data.Header_Description);
    ...
    <div className='h-full'>
        ...
        <div className='h-full w-full overflow-auto'>
            ...
            <thead>
                ...
                <th className='border-solid border-black border-[1px]'>Header Title</th>
                <th className='border-solid border-black border-[1px]'>Header Description</th>
            ...
            <tbody>
                {
                    props.infocardlist.map((item, index) => (
                        <tr className='border-solid border-black border-[1px] cursor-pointer hover:bg-[#2d34428c] hover:text-white' key = {index} onClick={editModal(item._id)}>
                            ...
                            <td className='border-solid border-black border-[1px]'>{item.Header_Title}</td>
                            <td className='border-solid border-black border-[1px]'>{item.Header_Description}</td>
                        ...
                        <form ref={formRef} encType="multipart/form-data">
                            <div className='grid grid-cols-2 md:grid-flow-row'>
                                <div className='col-span-2 md:col-span-1'>
                                    ...
                                    <div className='block m-4 justify-center sm:justify-between grid sm:flex px-1 my-8'>
                                        <label htmlFor="Header Title" className='mr-2'>Header Title:</label>
                                        <input value={headerTitle} name="headerTitle" onChange={(e) => setHeaderTitle(e.target.value)} type="text" className='rounded-[5px] border-[1px] border-black p-2' />
                                    </div>
                                    <div className='block m-4 justify-center sm:justify-between grid sm:flex px-1 my-8'>
                                        <label htmlFor="Header Description" className='mr-2'>Header Description:</label>
                                        <input value={headerDesc} name="headerDesc" onChange={(e) => setHeaderDesc(e.target.value)} type="text" className='rounded-[5px] border-[1px] border-black p-2' />
                                    </div>
                                </div>
                                <div className='col-span-2 md:col-span-1'>
